...
export function MarketPage({ coins, loading }: Props) {
  ...
  const selected   = coinSymbol ? (coins.find(c => c.symbol === coinSymbol) ?? null) : null

  function handleSelect(id: string) {
    const coin = coins.find(c => c.id === id)
    if (coin) setSearchParams({ coin: coin.symbol }, { replace: false })
  }
    ...
    <div className={`${classes.page} ${selected ? classes.detailOpen : ''}`}>
      <div className={classes.sidebar}>
        {loading
          ...
          : <CoinListView coins={coins} selectedId={selected?.id ?? null} onSelect={handleSelect} />
        }

        {isTableMobile && selected && (
          <div className={classes.inlineDetail}>
            <button className={classes.backBtn} onClick={handleDeselect}>← Назад</button>
      ...
      <div className={classes.main}>
        <AnimatePresence mode="wait">
          {selected ? (
            ...
            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={classes.empty}>
